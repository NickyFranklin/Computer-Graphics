...
/** Initialize GLFW and GLEW using reasonable settings. Exits on
    ...
    @param width The initial width of the window. If negative,
    automatically use a sensible default instead.

    @param height The initial height of the window. If negative,
    automatically use a sensible default instead.
...
float kuhl_make_label_8x8(const char *label, GLuint *texName, float color[3], float bgcolor[4], float pointsize)
{
	...
	// Count lines, record longest line length.
	int lines = 1;  // number of lines.  Is 1 if no \n is in string.
	int prevNewlineIdx = -1; // index of previously seen \n  (before start of string, in theory)
	int longestLineLength = 0;   // chars in longest line
	for(int i=0; i<len; i++)
	{
		if(label[i] == '\n')
		{
			lines++;
			// subtract 1 since two newlines in a row should mean a 0-length string in this line.
			int thisLineLength = i-prevNewlineIdx-1;
	...
	if(len-prevNewlineIdx-1 > longestLineLength)  	// If string ends without a \n at the end.
		longestLineLength = len-prevNewlineIdx-1;
	//printf("Lines: %d\n", lines);
	//printf("Longest line: %d\n", longestLineLength);

	// Width and height of output texture in texels
	int width = 8*longestLineLength;
	int height = 8*lines;

	// Always output a 8x8 texture or larger. Even if an empty string or \n\n\n
	// is passed in.
	if(width < 8)
		width = 8;
	if(height < 8)
	...
	// Initialize texture to background color
	unsigned char *image = malloc(width*height*4);
	for(int i=0; i<width*height*4; i+=4)
	{
		image[i+0] = bgcolor[0]*255;
		image[i+1] = bgcolor[1]*255;
		image[i+2] = bgcolor[2]*255;
		image[i+3] = bgcolor[3]*255;
	}

	// Row/col are in characters with the top left character being row=0, col=0
	int currentRow = 0;
	int currentCol = 0;
	for(int c=0; c<len; c++) // for each character
	{
		...
		// Calculate index of lower left pixel in this character.
		int llx = currentCol*8;   // horizontal distance measured in characters printed on this line.
		int lly = height - (currentRow+1)*8; // vertical distance starts 8 pixels below top of iamge.
		int ll = (lly*width + llx)*4;
		currentCol++;
		...
		for(int x=0; x<8; x++) // for each row
		{
			for(int y=0; y<8; y++) // for each column
			{
				...
				/*
				  // DEBUG
				if(start >= width*height*4)
				{
					printf("out of bounds %d > %d\n", start, width*height*4);
				}
				*/
				if(bits >> x & 1)
				{
...
/*
  // Debug: Write red dot in lower left corner of each character.
				if(x == 0 && y==0 )
				{
					image[start+0] = 255;
					image[start+1] = 0;
