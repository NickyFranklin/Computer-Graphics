...
void display()
{
	...
	if(dgr_is_master())
	{
		...
		if(lasttime == 0 || now - lasttime > 200) // reduce number to increase frequency of FPS label updates.
		{
			...
			char message[1024];
			snprintf(message, 1024, "FPS: %0.2f\n%s", fps, glGetString(GL_RENDERER)); // make a string with fps in it
	...
	for(int viewportID=0; viewportID<viewmat_num_viewports(); viewportID++)
	{
		...
		if(dgr_is_master())
		{
			float heightPercent = 1/20.0f; // Vertical percentage of window label should use up.
			float stretchLabel[16];
			mat4f_scale_new(stretchLabel, heightPercent*2 / viewmat_window_aspect_ratio(), heightPercent*2, 1.0f);
			
			/* Position label in the upper left corner of the screen */
			float transLabel[16];
			mat4f_translate_new(transLabel, -1+.01f, 1-heightPercent*2 - .01f, 0.0f);
