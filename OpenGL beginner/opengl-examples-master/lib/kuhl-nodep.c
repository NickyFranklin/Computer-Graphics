...
char* kuhl_find_file(const char *filename)
{
	...
	for(int i=0; i<origLen; i++)
		if(commonDirs[i][0] != '/')
		{
			// Make a copy of original string to avoid compiler
			// warnings about destination overlapping with the path in
			// commonDirs that we are reading from.
			char *this_path = strdup(commonDirs[i]);
			int retVal = snprintf(commonDirs[commonDirsLen++], 256, "%s/%s", dir, this_path);
			free(this_path);
			if(retVal >= 256)
