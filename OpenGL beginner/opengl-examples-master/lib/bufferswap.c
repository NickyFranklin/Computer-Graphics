...
#define FPS_SAMPLES 40
	...
	if(listfull == 1)
	{
		...
		/* Set bufferswap.logmissedframes to 1 in configuration to get this message */
		if(kuhl_config_int("bufferswap.logmissedframes", 0, 0) && timeLastFrame > expectedTimePerFrame*1.5)
		{
...
static void bufferswap_latencyreduce()
{
	...
	msg(MSG_INFO, "LastRender=%d AvgRender=%d AvgRenderDev=%d VsyncWait=%d AvgVsyncWait=%d MissedBy=%ld",
...
static void bufferswap_init(void)
{
	   ...
	   !glfwExtensionSupported("WGL_EXT_swap_control_tear"))
	{
		...
		{
			msg(MSG_INFO, "Machine lacks support for swap_control_tear extension. Swapping buffers during monitor refresh.");
...
void bufferswap(void)
{
	...
	/* Swap the buffers */
	if(viewmat_swapinterval == 0 ||
	   kuhl_config_boolean("bufferswap.latencyreduce", 0,0) == 0) // if FPS is unrestricted.
		bufferswap_simple();
	else
