...
int main(int argc, char *argv[]) {
  ...
  //Set up array for my test image
  ...
  //Start of the checkboard code
  FILE *checkerBoard;
  checkerBoard = fopen("checkboard.png", "w");

  //since we know that the image will be 512 x 512, I just figured out the array size
  ...
  for(int i = 0; i < assignmentSize; i++) {
    pixelX = xCoord(i);
    pixelY = yCoord(i);
    //If it fits these parameters, then it should be red starting the row
    if(pixelY % 128 < 64) {
      //Starts with a red square and goes to blue later
      if(pixelX % 128 < 64) {
	if(i % 3 == 0) {
	  pixelArray[i] = 255;
	}
